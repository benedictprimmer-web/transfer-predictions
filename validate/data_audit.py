...
def warehouse_inventory(con: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows).sort_values("table_or_view").reset_index(drop=True)
...
def fee_selection(con: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    ...
    from transfers_canonical
    group by 1,2,3,4,5
    order by row_count desc, era, to_league, pos_group, age_band, transfer_type
    limit 500
    """
    return con.execute(q).fetchdf()
...
def timestamp_gaps(con: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    ...
    from transfers_canonical
    group by 1,2,3
    order by row_count desc, date_source, mv_source, mv_is_point_in_time
    """
    return con.execute(q).fetchdf()
...
def coverage(con: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    ...
    from transfers_canonical
    group by 1,2,3
    order by row_count desc, position_or_role, to_league, age_band
    limit 500
    """
    return con.execute(q).fetchdf()
        ...
        _write_json(
            {
                ...
                "determinism_note": "Manifest omits commit and branch so tracked reports can be regenerated byte-for-byte across commits.",
                "outputs": artifacts,
            },
            "run_manifest.json",
        ).relative_to(REPO)
